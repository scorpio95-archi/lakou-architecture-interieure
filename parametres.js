...
async function init(){
  ...
  const { data: profile, error } = await sb
    .from('profiles')
    .select('full_name, role, school, school_id, avatar_url, bio, location')
  ...
  settingsWrap.innerHTML = `
    ...
    <form id="profileForm">
      ...
      <div class="form-group">
        ...
        <textarea id="p-bio" placeholder="Quelques lignes sur toi (visible si tu es admin, sur la page d'accueil)." style="min-height:80px;">${escapeHtml(p.bio)}</textarea>
      </div>

      <div class="form-group">
        <label for="p-location">Localisation</label>
        <input type="text" id="p-location" placeholder="Ville, pays..." value="${escapeHtml(p.location)}">
      </div>
